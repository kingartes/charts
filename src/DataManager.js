...
class DataManager  {
    ...
    getChartVertices(chartVertices, chartIdx, params) {
        ...
        for ( let ii = 0; ii < chartVertices.length; ii++) {
            ...
            const newPos = this.convertDataToPos(posX, posY, params.minX, params.maxX, params.maxY)
    ...
    buildLetterBuffer({ lines, labels }, maxY, minX, maxX) {
        const labelsBuffers = []
        for (let kk=0; kk < labels.length; kk++) {
            ...
            for (let ii = 0; ii < label.length; ii++) {
                ...
                height =  maxY / height
                const vertices = [
                    minX - width / 4 + ii * width / 3, lineY, 1,
                    minX - width / 4 + ii * width / 3+ width, lineY, 1,
                    minX - width / 4 + ii * width / 3 + width, lineY + height, 1,
                    minX - width / 4 + ii * width / 3+ width, lineY + height, 1,
                    minX - width / 4+ ii * width  / 3, lineY  + height, 1,
                    minX - width / 4+ ii * width  / 3, lineY, 1
                ]
                const colors = [
                    100, 100, 100,
    ...
    buildCircles(maxY, minX, maxX) {
        let vertices = [[], []]
        const deltaX = maxX -minX
        for (let ii = 0; ii <= 360; ii++) {
            const rad = ii * Math.PI / 180
            vertices[0] = vertices[0].concat([minX + Math.sin(rad) * deltaX / 4, Math.cos(rad) * maxY / 4, 1])
            vertices[0] = vertices[0].concat([minX + Math.sin(rad) * deltaX / 4 * 0.95, Math.cos(rad) * maxY / 4  * 0.95, 1])
            vertices[1] = vertices[1].concat([minX + Math.sin(rad)* 0.95, Math.cos(rad) * 0.95, 1])
            vertices[1] = vertices[1].concat([minX + deltaX / 2 , maxY / 2, 1])
        }
        return [
    ...
    setChartData () {
        ...
        const lines = this.buildLines(maxY, minX, maxX)
        const circlesBuffers = this.buildCircles(maxY, minX, maxX)

        // console.log(maxY, 'datachart')
        AnimationController.AddAnimation('MaxY', maxY, maxY, 2*maxY)
        ...
        this.dataToDraw = {
            ...
            lines,
            circlesBuffers,
            letterInfo: this.buildLetterBuffer(lines, maxY,minX, maxX)
        }
    }
}
